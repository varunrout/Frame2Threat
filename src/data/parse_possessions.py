...
def build_possession_sequences(
    ...
    *,
    ...
    """
    required = {
        "match_id", "possession_id", "index", "period",
        "minute", "second", "type_name", "team_name",
        "play_pattern_name", "location_x", "location_y",
        "under_pressure",
    }
    ...
    rows: list[dict] = []
    for (match_id, poss_id), grp in df.groupby(
...
def _aggregate_possession(
    ...
    territory_gained = float(max_x_reached - start_x) if (
        not np.isnan(max_x_reached) and not np.isnan(start_x)
    ) else np.nan
...
_SEQUENCE_TYPES = frozenset({
    "pass", "carry", "ball receipt*", "ball receipt",
    "dribble", "shot", "pressure", "duel", "clearance",
    "interception", "block", "foul committed", "foul won",
...
def _set_dtypes(df: pd.DataFrame) -> None:
    ...
    int_cols = [
        "match_id", "possession_id", "period",
        "n_events", "n_passes", "n_carries",
        "n_pressures_faced", "duration_seconds",
    ]
    for c in int_cols:
        if c in df.columns:
    ...
    float_cols = [
        ...
        "max_x_reached", "territory_gained",
        "mean_pass_length",
    ]
    for c in float_cols:
        if c in df.columns:
...
if __name__ == "__main__":
    ...
    label_cols = [
        "poss_has_shot", "poss_entered_box", "poss_entered_final_third",
        "poss_dangerous", "poss_broke_pressure", "poss_bypassed_lines",
        "poss_recycled",
    ]
    print(f"Label rates:")
    ...
    lens = poss["n_events"]
    print(f"  mean={lens.mean():.1f}  median={lens.median():.0f}  "
