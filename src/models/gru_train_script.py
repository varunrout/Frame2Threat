...
X_tr, L_tr = build_sequence_tensors(tr)
y_tr = tr[LABEL].astype(int).values
X_va, L_va = build_sequence_tensors(va)
y_va = va[LABEL].astype(int).values
X_te, L_te = build_sequence_tensors(te)
...
test_loader = make_dataloader(
    X_te, L_te, y_te, X_tab=X_tab_te, batch_size=TRAIN["test_batch_size"], shuffle=False
)
...
print()
print(
    classification_report(y_te, (test_proba >= 0.5).astype(int), target_names=["safe", "dangerous"])
)
