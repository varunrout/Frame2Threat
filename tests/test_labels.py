...
def _make_events_for_possession(n: int = 20, seed: int = 42) -> pd.DataFrame:
    ...
    for i in range(n):
        ...
        rows.append({
            ...
            'location_x': start_x,
            'location_y': start_y,
            'pass_end_x': start_x + float(rng.uniform(5, 20)) if type_name == 'Pass' else np.nan,
            'pass_end_y': start_y + float(rng.uniform(-5, 5)) if type_name == 'Pass' else np.nan,
            'start_x': start_x,
            'start_y': start_y,
...
class TestDownstreamLabels:
    ...
    def test_all_columns_present(self):
        ...
        expected = {'final_third_entry_k', 'box_entry_k', 'shot_within_k', 'dangerous_progression_k'}
